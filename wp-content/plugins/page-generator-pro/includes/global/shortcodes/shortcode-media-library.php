...
class Page_Generator_Pro_Shortcode_Media_Library extends Page_Generator_Pro_Shortcode_Base {
	...
	public function get_overview() {
		...
		return array(
			...
			'gutenberg_icon'              => $this->base->get_class( 'common' )->file_get_contents( $this->base->plugin->folder . '/assets/images/icons/admin-media.svg' ),
		);
	...
	public function render( $atts ) {
		...
		// If a caption is specified, append it to the image now.
		if ( $atts['output_caption_display'] ) {
			$html  = '<figure class="wp-block-image size-' . $atts['size'] . '">' . $html;
			$html .= '<figcaption>' . $atts['output_caption'] . '</figcaption>';
			$html .= '</figure>';
		}
