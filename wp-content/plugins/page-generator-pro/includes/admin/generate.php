...
class Page_Generator_Pro_Generate {
	 ...
	 * @param   string $system             System (browser|cli).
	...
	public function generate( $group_id, $type = 'content', $resume_index = 0, $number_of_posts = 0, $test_mode = false, $system = 'browser' ) {
		...
		if ( $groups->is_generating( $group_id ) ) {
			$error = new WP_Error(
				...
				sprintf(
					/* translators: %1$s: Group ID, %2$s: System (browser or cli) */
		...
		$this->add_to_debug_log(
			sprintf(
				/* translators: %1$s: Group ID, %2$s: System (browser,cli) */
	...
	private function build_search_replace_arrays_for_keyword( $keyword_with_modifiers, $keyword, $term ) {
		...
		if ( $keyword_with_modifiers == $keyword ) { // phpcs:ignore WordPress.PHP.StrictComparisons
	...
	private function apply_nearby_keyword_transformation( $term, $keyword, $column, $latitude_column, $longitude_column, $radius = 3, $order_by_distance = false ) {
		...
		foreach ( $this->keywords['terms'][ $keyword ] as $row => $keyword_term ) {
			...
			if ( trim( $term_parts[ $column_index ] ) != $term ) { // phpcs:ignore WordPress.PHP.StrictComparisons
				continue;
			}
		...
		foreach ( $this->keywords['terms'][ $keyword ] as $row => $keyword_term ) {
			...
			if ( $value == $term ) { // phpcs:ignore WordPress.PHP.StrictComparisons
				continue;
			}
			...
			switch ( $taxonomy->hierarchical ) {
				case true:
					foreach ( $taxonomy_terms[ $taxonomy->name ] as $tax_id => $terms_string ) {
						...
						if ( $tax_id != 0 ) { // phpcs:ignore WordPress.PHP.StrictComparisons
							$terms[] = (int) $tax_id;
							continue;
						}
