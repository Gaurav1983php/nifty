...
class Page_Generator_Pro_SpinnerChief extends Page_Generator_Pro_API {
	...
	public function text_with_spintax( $text, $params = array(), $protected_words = false ) {
		...
		$result = $this->response(
			$this->post( http_build_query( $params ), base64_encode( $text ) ) // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions
		);
	...
	private function response( $response ) {
		...
		// Decode body.
		$data = base64_decode( $response ); // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions
