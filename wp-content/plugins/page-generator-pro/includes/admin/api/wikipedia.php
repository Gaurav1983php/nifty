...
class Page_Generator_Pro_Wikipedia {
	...
	private function get_page( $term, $language ) {
		...
		@$dom->loadHTML( '<html><body>' . $html . '</body></html>' ); // phpcs:ignore WordPress.PHP.NoSilencedErrors
	 ...
	 * @return  mixed                       WP_Error | string
	 */
	private function get_similar_term( $dom, $language = 'en' ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter
	...
	private function get_similar_page_terms( $content ) {
		...
		foreach ( $similar_pages as $similar_page ) {
			foreach ( $similar_page->childNodes as $link ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
				// Skip if not a link.
				if ( $link->nodeName !== 'a' ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
					continue;
				}
				...
				$terms[] = $link->nodeValue; // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
			}
		}
	...
	private function get_headings( $dom ) {
		...
		foreach ( $table_of_contents as $heading ) {
			...
			foreach ( $heading->childNodes as $link ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
				// Skip if not a link.
				if ( $link->nodeName !== 'a' ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
					continue;
				}
				...
				if ( ! $heading_parts->length ) {
					$headings[ str_replace( '#', '', $link->getAttribute( 'href' ) ) ] = $link->nodeValue; // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
					continue;
				}
				...
				foreach ( $heading_parts as $heading_part ) {
					...
					$headings[ str_replace( '#', '', $link->getAttribute( 'href' ) ) ] = $heading_part->nodeValue; // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
					continue;
				}
			}
		}
	...
	private function is_element( $node, $search ) {
		...
		if ( $node->childNodes ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
