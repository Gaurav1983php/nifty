...
class Page_Generator_Pro_WooCommerce extends Page_Generator_Pro_Integration {
	...
	public function enqueue_product_js() {
		...
		// Define any WooCommerce fields here that use wc_format_decimal() and we want to support Keywords for.
		// wc_format_decimal() is called as part of the set_prop() process when populating a WC Product,
		// which we cannot override due to protected methods.
		// The underlying data is stored in the Post Meta table correctly as e.g. a Keyword, so we use JS
		// to 'late' fill the input fields to show this.
		wp_localize_script(
	 ...
	 * Keyword due to the property being a numerical format.
	 *
	 * @since   3.6.4
	 *
	 ...
	 */
	public function save_product_prices( $post_id, $post ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter
		...
		// General.
		update_post_meta( $post_id, '_regular_price', $_REQUEST['_regular_price'] );
		update_post_meta( $post_id, '_sale_price', $_REQUEST['_sale_price'] );
		delete_post_meta( $post_id, '_price' ); // Deliberate; this stores either the regular or sale price depending on conditions set.

		// Inventory.
		update_post_meta( $post_id, '_stock', $_REQUEST['_stock'] );

		// Shipping.
		update_post_meta( $post_id, '_weight', $_REQUEST['_weight'] );
		update_post_meta( $post_id, '_length', $_REQUEST['_length'] );
		update_post_meta( $post_id, '_width', $_REQUEST['_width'] );
		update_post_meta( $post_id, '_height', $_REQUEST['_height'] );
		// phpcs:enable
