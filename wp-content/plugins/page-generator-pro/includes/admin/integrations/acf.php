...
class Page_Generator_Pro_ACF extends Page_Generator_Pro_Integration {
	...
	public function match_term_group_location_rule( $match, $rule, $options, $field_group = false ) {
		...
		$taxonomy = sanitize_text_field( $_REQUEST['taxonomy'] ); // phpcs:ignore WordPress.Security.NonceVerification
		$term_id  = absint( sanitize_text_field( $_REQUEST['tag_ID'] ) ); // phpcs:ignore WordPress.Security.NonceVerification
