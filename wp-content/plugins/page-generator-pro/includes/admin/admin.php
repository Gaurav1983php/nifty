...
class Page_Generator_Pro_Admin {
	...
	public function __construct( $base ) {
		...
		add_action( 'page_generator_pro_setting_panel_page-generator-pro-georocket', array( $this, 'settings_screen_georocket' ), 1 );
	...
	public function settings_screen() {
		...
		// Get active tab.
		$current_tab = ( isset( $_GET['tab'] ) ? $_GET['tab'] : $this->base->plugin->name . '-general' ); // phpcs:ignore WordPress.Security.NonceVerification
		...
		switch ( $cmd ) {
			...
			case 'form-locations':
				...
				$keyword = array(
					...
					'method'       => $this->base->get_class( 'settings' )->get_setting( $this->base->plugin->name . '-georocket', 'method', 'radius' ),
					'location'     => '',
				...
				if ( isset( $_POST[ $this->base->plugin->name . '_nonce' ] ) && wp_verify_nonce( sanitize_key( $_POST[ $this->base->plugin->name . '_nonce' ] ), 'generate_locations' ) ) {
					...
					$keyword['orderby']  = wp_unslash( sanitize_text_field( $_POST['orderby'] ) );
					$keyword['order']    = wp_unslash( sanitize_text_field( $_POST['order'] ) );
					$keyword['method']   = wp_unslash( sanitize_text_field( $_POST['method'] ) );
					$keyword['location'] = wp_unslash( sanitize_text_field( $_POST['location'] ) );
					$keyword['radius']   = wp_unslash( sanitize_text_field( $_POST['radius'] ) );
				}
			...
			case 'form-phone':
				...
				if ( isset( $_POST[ $this->base->plugin->name . '_nonce' ] ) && wp_verify_nonce( sanitize_key( $_POST[ $this->base->plugin->name . '_nonce' ] ), 'generate_phone_area_codes' ) ) {
					$keyword['keyword']     = wp_unslash( sanitize_text_field( $_POST['keyword'] ) );
					$keyword['country']     = wp_unslash( sanitize_text_field( $_POST['country'] ) );
					$keyword['output_type'] = ( isset( $_POST['output_type'] ) ? stripslashes_deep( $_POST['output_type'] ) : '' );
				}
	...
	public function generate_locations() {
		...
		$args = array(
			...
			// Order By and Order.
			'orderby'      => isset( $_POST['orderby'] ) ? sanitize_sql_orderby( $_POST['orderby'] ) : false,
