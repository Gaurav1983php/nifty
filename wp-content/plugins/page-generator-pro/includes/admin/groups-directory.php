...
class Page_Generator_Pro_Groups_Directory {
	...
	private function process_form() {
		...
		$this->configuration = ( isset( $_REQUEST['configuration'] ) ? json_decode( stripslashes( $_POST['configuration'] ), true ) : array( // phpcs:ignore WordPress.Security.NonceVerification
			...
			case 1:
				...
				$this->configuration = array_merge(
					...
					array(
						...
						'service_keyword'     => stripslashes( sanitize_text_field( $_POST['service_keyword'] ) ),
						'service_keyword_id'  => '',
						'services'            => stripslashes( sanitize_textarea_field( $_POST['services'] ) ),
						'location_keyword'    => '',
						'location_keyword_id' => '',
						...
						'zipcode'             => stripslashes( sanitize_text_field( $_POST['zipcode'] ) ),
						'exclusions'          => ( ! empty( $_POST['exclusions'] ) ? explode( ',', sanitize_text_field( $_POST['exclusions'] ) ) : false ),
						'population_min'      => ( ! empty( $_POST['population_min'] ) ? (int) sanitize_text_field( $_POST['population_min'] ) : false ),
						'population_max'      => ( ! empty( $_POST['population_max'] ) ? (int) sanitize_text_field( $_POST['population_max'] ) : false ),
	...
	private function is_groups_directory_request() {
		...
		if ( sanitize_text_field( $_GET['page'] ) !== $this->base->plugin->name . '-groups-directory' ) { // phpcs:ignore WordPress.Security.NonceVerification
			return false;
		}
	...
	private function setup_location_keyword() {
		...
		if ( $this->configuration['exclusions'] !== false ) {
			$args['city_name_not'] = $this->configuration['exclusions'];
		}
		...
		switch ( $this->configuration['method'] ) {
			...
			case 'area':
				...
				if ( ! isset( $args['region_id'] ) && ! isset( $args['county_id'] ) ) {
					return new WP_Error(
						'page_generator_pro_groups_directory_setup_location_keyword',
