...
class Page_Generator_Pro_Access {
	...
	private function is_logged_in_user_always_permitted() {
		...
		foreach ( $this->permitted_users as $permitted_user ) {
			...
			if ( $this->current_user->user_login == $permitted_user ) { // phpcs:ignore WordPress.PHP.StrictComparisons
				return true;
			}
			if ( $this->current_user->user_email == $permitted_user ) { // phpcs:ignore WordPress.PHP.StrictComparisons
				return true;
			}
			if ( $this->current_user->ID == $permitted_user ) { // phpcs:ignore WordPress.PHP.StrictComparisons
				return true;
			}
		}
