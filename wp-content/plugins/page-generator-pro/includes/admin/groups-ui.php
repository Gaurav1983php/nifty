...
class Page_Generator_Pro_Groups_UI {
	...
	public function output_posts_filter_by_group_dropdown( $post_type ) {
		...
		$current_group_id = ( isset( $_REQUEST['page_generator_pro_group_id'] ) ? sanitize_text_field( $_REQUEST['page_generator_pro_group_id'] ) : '' ); // phpcs:ignore WordPress.Security.NonceVerification
	...
	public function posts_filter_by_group( $query ) {
		...
		if ( sanitize_text_field( $_REQUEST['post_type'] ) !== $query->query_vars['post_type'] ) { // phpcs:ignore WordPress.Security.NonceVerification
			return $query;
		}

		// Get Group ID.
		$group_id = sanitize_text_field( $_REQUEST['page_generator_pro_group_id'] ); // phpcs:ignore WordPress.Security.NonceVerification
		...
		if ( $group_id == -1 ) { // phpcs:ignore WordPress.PHP.StrictComparisons
			$query->query_vars['meta_query'][] = array(
