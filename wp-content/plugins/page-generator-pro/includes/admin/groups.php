...
class Page_Generator_Pro_Groups {
	...
	public function validate( $id ) {
		...
		// If the Parent Page isn't empty, check it's valid and free of e.g. quotation marks.
		if ( is_array( $settings['pageParent'] ) &&
			isset( $settings['pageParent'][ $settings['type'] ] ) &&
			! empty( $settings['pageParent'][ $settings['type'] ] ) ) {
	 ...
	 * which are displayed in the WP_List_Table and when editing the Content Group.
	 *
	 * @since   3.5.4
	 *
	 * @param   int  $id            Group ID.
	...
	/**
	 * Gets the status of the given Group ID (idle, generating)
	 *
	 * @since   1.9.9
	 *
	 * @param   int $id     Group ID.
